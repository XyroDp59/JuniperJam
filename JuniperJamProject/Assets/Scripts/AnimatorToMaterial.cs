   ...
   /* // URP/Lit property IDs
    ...
    void Update()
    {
        _material.mainTextureOffset = new Vector2(
            (xBaseOffset + spriteRenderer.sprite.rect.x) / widthImage,
            (yBaseOffset + spriteRenderer.sprite.rect.y) / heightImage
        );

        if (_spinRemainingDuration > 0)
        {
            transform.RotateAround(transform.position, Vector3.up,
                -Time.deltaTime * 360 * _numberOfSpin / _spinTotalDuration);
